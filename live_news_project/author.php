      ...
      <div class="container">
        <div class="row">
            <div class="col-md-8">
                ...
                $query_aut="SELECT*FROM user WHERE user_id={$author_id}";
                $result_aut=mysqli_query($conn,$query_aut) or die("query failed");
                $row_aut=mysqli_fetch_assoc($result_aut);
                ?>
                <h2 class="page-heading"><?php echo $row_aut['username']?></h2>

              <?php
              //include "admin/config.php";
              // $rcv_author=$_GET['authorid'];
              $limit=3;
              if(isset($_GET['page'])){
          ...
          LEFT JOIN user ON post.author=user.user_id WHERE post.author={$author_id} ORDER BY post.post_id DESC LIMIT {$offset},{$limit}";
